  ...
  return NextResponse.json({
    ...
    services: [
      ...
      { id: "search", method: "GET", url: `${BASE}/api/search`, price_usd: "0.01",
        summary: "Live web search -> organic results plus ads, People Also Ask, related searches, knowledge graph, shopping, and AI Overview when the Google-backed provider answers.",
        params: { q: "required", count: "number (max 20)", country: "2-letter code (optional)", language: "2+ letter code (optional)", page: "1-based (optional)", device: "forwarded upstream if set (optional)" } },
      ...
      { id: "callerid", method: "GET", url: `${BASE}/api/callerid`, price_usd: "0.00",
      ...
      { id: "shield", method: "GET/POST", url: `${BASE}/api/shield`, price_usd: "0.00",
        ...
        params: {
          ...
          "POST block|unblock": "{ owner, signature, address, chain?, reason? }",
          "POST allow|unallow": "{ owner, signature, address, reason? }",
          "GET list": "Bearer -> { blocks[], allows[] }",
          "GET check": "?address=0x...&chain=base|ethereum, Bearer -> { blocked, source, recommendation, reasons[] }",
          "GET stats": "Bearer -> 7d / 30d / 90d counts by action and entity category",
        } },
    ],
    docs: `${BASE}/llms.txt`,
